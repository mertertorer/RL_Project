...
class PPOAgent:
    def __init__(self, env,episodes, max_timesteps,ent_const = 0, gamma=0.99, lr=5e-4, eps_clip=0.2, K_epochs=10, lambd=0.95, device="cpu", env_name='Pendulum-v1'):
        self.env = env
        self.gamma = gamma
        self.lr = lr
    ...
    def visualize(self, iter = 5):      
        ...
        if self.env_name == 'Pendulum-v1':
            ...
            plt.ylabel('Total Reward')
            plt.title('Performance of the trained agent')
            plt.savefig(f'plots/ppo_rewards_{self.env_name}_E_{self.episodes}_TS_{self.max_timesteps}_LR_{self.lr}.png')
            # close plot
            plt.close()
